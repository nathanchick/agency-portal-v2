...
export default function EditApiToken({apiToken, availableAbilities}: Props) {
    const {data, setData, put, processing, errors} = useForm({
        name: apiToken.name,
        abilities: apiToken.abilities || [],
        expires_at: apiToken.expires_at ? apiToken.expires_at.split('T')[0] : '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('api-tokens.update', apiToken.id));
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit API Token: ${apiToken.name}`}/>
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-4">
                        ...
                        <HeadingSmall
                            ...
                            description="Update token name and permissions"
                        />
                    </div>
