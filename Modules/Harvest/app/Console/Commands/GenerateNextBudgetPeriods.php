    ...
    public function handle(): int
    {
        ...
        foreach ($servicesNeedingPeriods as $service) {
            ...
            while ($currentPeriodEnd->lessThan(now()) && $periodsCreatedForService < $maxPeriodsToCreate) {
                ...
                // Get the actual last period for rollover calculation
                $actualLastPeriod = ServiceBudgetPeriod::where('service_id', $service->id)
                    ->orderBy('period_end', 'desc')
                    ->first();

                // Calculate rollover hours if enabled
                $rolloverHours = 0;
                if ($service->budget_rollover_enabled && $actualLastPeriod && $actualLastPeriod->reconciled) {
                    $remainingHours = ($actualLastPeriod->budget_hours + $actualLastPeriod->hours_rollover_from_previous) - $actualLastPeriod->hours_used;
                    if ($remainingHours > 0) {
                ...
                $newPeriod = ServiceBudgetPeriod::create([
                    ...
                    'hours_used' => 0,
                    'amount_used' => 0,
                    'hours_rollover_from_previous' => $rolloverHours,
                ]);
